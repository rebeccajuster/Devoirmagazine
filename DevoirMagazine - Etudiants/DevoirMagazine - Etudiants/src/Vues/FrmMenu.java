...
public class FrmMenu extends JFrame
{
    ...
    public FrmMenu()
    {
       ...
       mdl = new ModelJTable();
       mdl.LoadDatasMagazines(mesMagazines);
       tblMagazines.setModel(mdl);

       for(Pigiste p : mesPigistes)
       {
           cboPigistes.addItem(p.getNomPigiste());
       }
        ...
        tblMagazines.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                int numMag = Integer.parseInt(tblMagazines.getValueAt(tblMagazines.getSelectedRow(),0).toString());

                for(Magazine mgz : mesMagazines)
                {
                    if(numMag == mgz.getNumMagazine())
                    {
                        mdl = new ModelJTable();
                        mdl.LoadDatasArticles(mgz.getLesArticles());
                        tblArticles.setModel(mdl);
                        txtMontantMagazine.setText(String.valueOf(mgz.CalculerMontantMagazine()));
                        magazineSelectionne = mgz;
                        break;
        ...
        tblArticles.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                String nomArt = tblArticles.getValueAt(tblArticles.getSelectedRow(),0).toString();

                for(Article art :magazineSelectionne.getLesArticles())
                {
                    if(nomArt.equals(art.getTitreArticle()))
                    {
        ...
        btnAjouterArticle.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                {
                    JOptionPane.showMessageDialog(null,"Veuillez selectionner un magazine","Choix du magazine",JOptionPane.WARNING_MESSAGE);
                }
                else if (txtTitreArticle.equals(" "))
                {
                    JOptionPane.showMessageDialog(null,"Veuillez selectionner un titre","Erreur de saisie",JOptionPane.WARNING_MESSAGE);
                }
                else
                {
                    String nomArt = txtTitreArticle.getText();
                    String nomPigiste = cboPigistes.getSelectedItem().toString();
                    int nbPage = sldNbPages.getValue();
                    int numPigiste = 0;
                    double prixPigiste = 0;


                    for(Pigiste p : mesPigistes)
                    {
                        if(nomPigiste.equals(p.getNomPigiste()))
                        {
                            numPigiste = p.getNumPigiste();
                            prixPigiste = p.getPrixPage();
                        }
                    }

                    Pigiste lePig = new Pigiste (numPigiste,nomPigiste,prixPigiste);

                    Article nvlArt = new Article (nomArt,nbPage,lePig);
                    magazineSelectionne.getLesArticles().add(nvlArt);
                    txtMontantArticle.setText(String.valueOf(nvlArt.CalculerMontantArticle()));
                    txtMontantMagazine.setText(String.valueOf(magazineSelectionne.CalculerMontantMagazine()));


                    mdl = new ModelJTable();
                    mdl.LoadDatasArticles(magazineSelectionne.getLesArticles());
                    tblArticles.setModel(mdl);
